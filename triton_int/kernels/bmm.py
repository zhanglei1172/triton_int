...
def test_correct_int8(B=10, M=32, N=4096, K=4096):
    a = torch.randn((B, M, K), device="cuda", dtype=torch.float32)
    b = torch.randn((B, N, K), device="cuda", dtype=torch.float32)
    ...
    print(
        "Output cos",
        cos(
            triton_output4.flatten().to(torch.float32),
            torch_output.flatten().to(torch.float32),
        ),
    )
